...
from captchabreakerweb.models import DatasetModel, OriginalImageModel, db, ClasificatorModel, QueryModel
...
def overview():
    classifiers = ClasificatorModel.query.filter_by(is_finished=False).all()
    dataset_count = DatasetModel.query.count()
    classifiers_count = ClasificatorModel.query.count()
...
def clasificators():
    classifiers = ClasificatorModel.query.all()
...
def clasificators_new_upload():
    from captchabreakerweb.tasks.process_dataset import training_task
    print(request.form)
...
def classificators_detail(id):
    classifier = ClasificatorModel.query.get(id)
...
def longtask():
    from captchabreakerweb.tasks.process_dataset import long_task, short_task
