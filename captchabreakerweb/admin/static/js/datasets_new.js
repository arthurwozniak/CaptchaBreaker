...
fileInput.bind('change', function() {
    ...
    if (files.type !== "application/zip") {
        show_errors("Only ZIP files are allowed");
        fileInput.val("")
        stepNextBtn.toggleClass("disabled", true);
        return;
    }
...
// workaround when user types last captcha text and clicks outside
$("#text-captcha").change(function(e){
    console.log("FOO")
    labels[Object.keys(archiveEntries)[labelsPos]] = $("#text-captcha").val();
    if (stepNextBtn.hasClass('disabled') && (Object.keys(labels).length == labelsTotal)) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            for (i = 0; i < result.length; i++) {
                ...
                clone.find("img").attr("src", "data:image/png;base64," + result[i][1]).attr("id", "")
