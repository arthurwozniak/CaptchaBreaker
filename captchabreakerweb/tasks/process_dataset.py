...
from captchabreakerweb import celery, app
from captchabreakerweb.models import DatasetModel, ClasificatorModel, QueryModel, db

from captchabreakerweb.ml.CNN import CNN
from captchabreakerweb.ml.CaptchaBreakerDataset import CaptchaBreakerDataset
...
def training_task(self, name, dataset_id, accuracy, iterations, task_id=None):
    with app.app_context():
        clasificator = ClasificatorModel(name=name, task_id=self.request.id, dataset_id=dataset_id, is_finished=False)
